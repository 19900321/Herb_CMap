...
def get_shortest_path_length_between(G, source_id, target_id):
    return networkx.shortest_path_length(G, source_id, target_id, weight='w')
...
class Construct_Network:
    ...
    def __init__(self, file_name):
        ...
        # here the edge weight is true store_edge_type=True, use_edge_data = True
        self.setNode, self.setEdge, self.dictNode, self.dictEdge = self.get_nodes_and_edges_from_sif_file(store_edge_type=True, delim=None, data_to_float=True)
        self.g = self.create_network_from_sif_file( use_edge_data = True, delim = None, include_unconnected=True)
    ...
    def create_network_from_sif_file(self, use_edge_data = True, delim = None, include_unconnected=True):
...
#a = Construct_Network("source_data/toy.sif")